...
def rows() -> list[dict[str, object]]:
    ...
    for index in range(1000):
        ...
        result.append(
            {
                "offer_id": f"SYNTH-{index + 1:04d}",
                "platform_id": platform_id,
                "platform_name": platform_name,
                "offer_title": f"Synthetic scenario offer {index + 1}",
                "bank_id": bank_id,
                "bank_name": bank_name,
                "supported_cards": cards,
                "payment_method": payment,
                "category": "FLIGHT_DOMESTIC",
                "booking_channel": ("WEB", "APP", "WEB_AND_APP")[index % 3],
                "discount_type": discount_type,
                "discount_value": discount_value,
                "max_discount": "" if cap is None else cap,
