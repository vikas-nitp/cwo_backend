...
class InMemoryRateLimiter:
        ...
        if now - self._last_cleanup > self._cleanup_interval:
            ...
            for key in list(self._global_buckets.keys()):
                self._global_buckets[key] = self._cleanup_old_entries(
                    self._global_buckets[key], 
                    RateLimitConfig.GLOBAL_WINDOW_SEC
                )
            ...
            for key in list(self._search_buckets.keys()):
                self._search_buckets[key] = self._cleanup_old_entries(
                    self._search_buckets[key],
                    RateLimitConfig.SEARCH_WINDOW_SEC
                )
    ...
    def check_global_limit(self, request: Request) -> tuple[bool, int]:
        ...
        try:
            ...
            bucket = self._cleanup_old_entries(
                self._global_buckets[client_key],
                RateLimitConfig.GLOBAL_WINDOW_SEC
            )
    ...
    def check_search_limit(self, request: Request) -> tuple[bool, int]:
        ...
        try:
            ...
            bucket = self._cleanup_old_entries(
                self._search_buckets[client_key],
                RateLimitConfig.SEARCH_WINDOW_SEC
            )
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if path == "/api/v1/search" and method == "POST":
            ...
            if not allowed:
                ...
                return JSONResponse(
                    status_code=429,
                    content={"detail": "Too many search requests. Please wait."},
            ...
            if content_length:
                try:
                    if int(content_length) > self.MAX_BODY_SIZE:
                        return JSONResponse(
                            status_code=413,
                            content={"detail": "Request body too large (max 32KB)"}
                        )
                except ValueError:
                    pass
